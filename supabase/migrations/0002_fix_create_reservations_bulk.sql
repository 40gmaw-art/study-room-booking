-- Fix for: "Could not find the function public.create_reservations_bulk(...)"
-- This means the function from 0001_create_reservations_bulk.sql was never
-- actually applied to the live database (or PostgREST's schema cache never
-- picked it up). This migration is self-contained and safe to run on its
-- own, even if 0001 was never executed.
--
-- Steps:
--   1. Drop any existing overload(s) of create_reservations_bulk, regardless
--      of their exact parameter signature (guards against a stray/partial
--      version from an earlier attempt).
--   2. Recreate the function with parameter names that exactly match the
--      frontend RPC call in lib/booking-actions.ts:
--        p_reservation_date, p_start_times, p_name, p_department, p_student_number
--   3. Grant EXECUTE to authenticated.
--   4. Ask PostgREST to reload its schema cache immediately.
...
begin
  ...
  -- (never trust client-supplied counts/availability)
  foreach v_start_time in array v_sorted_times loop
  ...
  -- pass 4: all slots validated under lock -> insert every reservation.
  -- If anything above raised an exception, this point is never reached and
  -- the whole call (a single transaction) rolls back with nothing saved.
  foreach v_start_time in array v_sorted_times loop
...
-- Make PostgREST pick up the (re)created function immediately instead of
