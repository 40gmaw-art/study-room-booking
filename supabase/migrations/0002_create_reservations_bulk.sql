-- Creates public.create_reservations_bulk, verified against the actual
-- table/RPC definitions in supabase/schema.sql (public.reservations columns:
-- id, reservation_number, user_id, name, department, student_number,
-- reservation_date, start_time, status, created_at, updated_at, cancelled_at;
-- single-slot pattern taken from public.create_reservation). No column was
-- invented — every column referenced below already exists in schema.sql.
--
-- Safe to run standalone and safe to re-run: drops any pre-existing overload
-- of this function (regardless of signature) before recreating it, so it
-- never fails with "function already exists" / "cannot change return type".
